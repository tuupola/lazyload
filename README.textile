...
Lazy Load delays loading of images in long web pages. Images outside of the viewport won't be loaded before the user scrolls to them. This is the opposite of image preloading.

Using Lazy Load on long web pages containing many large images makes the page load faster. The browser will be in the ready state after loading visible images. In some cases, it can also help to reduce server load.

Lazy Load is inspired by the "YUI ImageLoader":http://developer.yahoo.com/yui/imageloader/ Utility by Matt Mlinac.
...
You must alter your HTML code. The URL of the real image must be put into the data-original attribute. It is a good idea to give Lazy Loaded images a specific class. This way you can easily control which images the plugin is binded to. Note that you should have the width and height attributes in your image tag.
